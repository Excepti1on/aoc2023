...
void Day16() {
    ...
    for (int i = 0; i < SIZE; ++i) {
        for (int j = 0; j < SIZE; ++j) {
            sum +=
                visited[i * SIZE + j + SIZE * SIZE * 0] | visited[i * SIZE + j + SIZE * SIZE * 1] |
                visited[i * SIZE + j + SIZE * SIZE * 2] | visited[i * SIZE + j + SIZE * SIZE * 3];
        }
    }
    printf("%d\n", sum);

    for (int j = 0; j < SIZE; ++j) {
        struct Beam *beams[4] = {&(struct Beam){0, j, RIGHT}, &(struct Beam){SIZE - 1, j, LEFT},
                                 &(struct Beam){j, 0, DOWN}, &(struct Beam){j, SIZE - 1, UP}};
        for (int a = 0; a < 4; ++a) {
...
void Path(struct Beam *beam, char buffer[SIZE][SIZE + 2], bool (*visited)[SIZE * SIZE * 4]) {
    while (true) {
        if ((*visited)[beam->y * SIZE + beam->x + beam->direction * SIZE * SIZE] ||
            beam->x >= SIZE || beam->y >= SIZE || beam->x < 0 || beam->y < 0) {
            break;
        }
