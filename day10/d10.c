...
void Day10() {
    ...
    int possibilities = 0;
    if (buffer[start_y - 1][start_x] == '|' || buffer[start_y - 1][start_x] == 'F' || buffer[start_y - 1][start_x] == '7') {
        x = start_x;
        y = start_y - 1;
        last_move = UP;
        possibilities |= UP;
    }
    if (buffer[start_y][start_x + 1] == '-' || buffer[start_y][start_x + 1] == '7' || buffer[start_y][start_x + 1] == 'J') {
        x = start_x + 1;
        y = start_y;
        last_move = RIGHT;
        possibilities |= RIGHT;
    ...
    if (buffer[start_y][start_x - 1] == '-' || buffer[start_y][start_x - 1] == 'L' || buffer[start_y][start_x - 1] == 'F') {
        x = start_x - 1;
        y = start_y;
        last_move = LEFT;
        possibilities |= LEFT;
    ...
    for (size_t i = 0; i < SIZE_Y; ++i) {
        for (size_t j = 1; j < SIZE_X; ++j) {
            ...
            for (size_t k = j; k < SIZE_X; ++k) {
                ...
                if (buffer[i][k] == '|' || buffer[i][k] == 'J' && last_direction == UP || buffer[i][k] == '7' && last_direction == DOWN) {
                    intersections++;
